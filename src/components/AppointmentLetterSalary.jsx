...
import { getEmployeeStructure, previewEmployeeStructure } from "../services/salaryComponentService";
import "./EmployeeSalaryStructureView.css";

const formatInr = (n) => `₹${Number(n || 0).toLocaleString("en-IN")}`;

const groupStructureComponents = (components = []) => {
  const earnings = components.filter((c) => c.category?.toLowerCase() === "earning" && c.enabled !== false && (c.monthlyAmount || 0) > 0);
  const deductions = components.filter((c) => c.category?.toLowerCase() === "deduction" && !c.isEmployerContribution && c.enabled !== false && (c.monthlyAmount || 0) > 0);
  const employerContributions = components.filter((c) => c.isEmployerContribution && c.enabled !== false && (c.monthlyAmount || 0) > 0);
...
export default function AppointmentLetterSalary({ employeeId }) {
  ...
  const earnings = preview?.earnings?.length ? preview.earnings.map(c => ({...c, amount: c.amount * mult})).filter(c => (c.amount ?? 0) > 0) : fallback.earnings.map((c) => ({ ...c, amount: c.monthlyAmount * mult })).filter(c => (c.amount ?? 0) > 0);
  const deductions = preview?.deductions?.length ? preview.deductions.map(c => ({...c, amount: c.amount * mult})).filter(c => (c.amount ?? 0) > 0) : fallback.deductions.map((c) => ({ ...c, amount: c.monthlyAmount * mult })).filter(c => (c.amount ?? 0) > 0);
  const employerContributions = preview?.employerContributions?.length ? preview.employerContributions.map(c => ({...c, amount: c.amount * mult})).filter(c => (c.amount ?? 0) > 0) : fallback.employerContributions.map((c) => ({ ...c, amount: c.monthlyAmount * mult })).filter(c => (c.amount ?? 0) > 0);
  ...
  if (isCalendarDaily) {
    ...
    const monthlyGross = Number(structure?.monthlyGross || fallback.earnings.reduce((s, c) => s + (Number(c.monthlyAmount) || 0), 0));
    const totalDed = deductions.reduce((s, d) => s + (d.amount ?? 0), 0);
    const netMonthly = Math.max(0, monthlyGross - totalDed);
    const ctcTarget = monthlyGross || 1;
    const dailyEarnings = earnings.map(c => ({ ...c, amount: c.monthlyAmount || 0 }));
    ...
    return (
      <div className="emp-salary-view">
        ...
        <CtcBreakdownBar earnings={dailyEarnings} ctcTarget={ctcTarget} />
        <div className="emp-salary-view__panels">
          <ViewPanel title="Earnings (monthly preview)" items={dailyEarnings} emptyText="No earnings configured." ctcTarget={ctcTarget} />
  ...
  if (isDaily) {
    ...
    const netDaily = Math.max(0, dailyGross - totalDed);
      ...
      <div className="emp-salary-view">
        ...
        <div className="emp-salary-view__summary">
          ...
          <div className="emp-salary-view__summary-row">
            ...
            <strong className="emp-salary-view__summary-value" style={{color: '#dc2626'}}>-{formatInr(totalDed)}/day</strong>
          </div>
